...
public class ResetRowHideAction extends AbstractColumnRowIndexAction
{
    ...
    public ResetRowHideAction(ElementCasePane elementcasepane, int i)
    {
    ...
    protected boolean executeActionReturnUndoRecordNeededWithCellSelection(CellSelection cellselection)
    {
        ...
        {
            int k = ai1[j];
            if(templateelementcase.getRowHeight(k).equal_zero())
            {
                templateelementcase.setRowHeight(k, UNITConstants.DEFAULT_ROW_HEIGHT);
                flag = true;
            }
        }
    ...
    public void update()
    {
        ...
        if(selection instanceof CellSelection)
        {
            ...
            for(int i = 0; i < ai.length; i++)
                if(templateelementcase.getRowHeight(i).equal_zero())
                {
                    setEnabled(true);
                    return;
                }
